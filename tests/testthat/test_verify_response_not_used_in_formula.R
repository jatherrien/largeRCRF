...
test_that("CompetingRisks - response not included", {
  ...
  forest <- train(CR_Response(delta, Time) ~ ., sampleData, ntree=50, numberOfSplits=0, mtry=1, nodeSize=5, cores=1)
...
test_that("CompetingRisksWithCensorTimes - response not included", {
  ...
  forest <- train(CR_Response(delta, Time, CensorTime) ~ ., sampleData, ntree=50, numberOfSplits=0, mtry=1, nodeSize=5, cores=1)
...
test_that("Regression - response not included", {
  ...
  forest <- train(y ~ ., sampleData, ntree=50, numberOfSplits=0, mtry=1, nodeSize=5, cores=1)
...
test_that("Regression - response not included (with Numeric)", {
  ...
  forest <- train(Numeric(y) ~ ., sampleData, ntree=50, numberOfSplits=0, mtry=1, nodeSize=5, cores=1)
